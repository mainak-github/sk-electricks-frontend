...
export default function ApprovedCustomerList() {
  ...
  // Fetch approved customers from API
  const fetchApprovedCustomers = async (page = 1, search = '', filter = filterType) => {
    setLoading(true)
    setError(null)
    try {
      ...
      const response = await fetch(`${config.API_URL}/customers/approved?${queryParams}`)
      if (!response.ok) {
  ...
  // Handle filter type change dynamically
  const handleFilterChange = (newFilter) => {
    setFilterType(newFilter)
    if (showTable) {
      fetchApprovedCustomers(1, '', newFilter)
    }
  }

  const handleGetList = () => {
    setShowTable(true)
    fetchApprovedCustomers(1, '', filterType)
  }
  ...
  const handleStatusUpdate = async (customerId, customerName, newStatus, rejectionReason = '') => {
    try {
      ...
      alert(`Customer "${customerName}" status updated to ${newStatus}`)
      // Refresh list to reflect status update and filter
      fetchApprovedCustomers(pagination.page, '', filterType)
    } catch (err) {
  ...
  // Approve/Pending/Reject/View actions
  const handleAction = (action, customerId, customerName) => {
    if (action === 'Pending') {
      if (confirm(`Move "${customerName}" back to pending?`)) {
        handleStatusUpdate(customerId, customerName, 'pending')
      }
  ...
  const handlePageChange = (newPage) => {
    fetchApprovedCustomers(newPage, '', filterType)
  }
      ...
      <div className="bg-white rounded-lg shadow-sm border">
        ...
        <div className="p-6">
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <button
              onClick={handlePrint}
              disabled={!showTable || approvedCustomers.length === 0}
          ...
          {loading && (
            <div className="text-center py-8">
              ...
              <p className="mt-2 text-gray-600">Loading approved customers...</p>
            </div>
          )}
            ...
            <>
              {approvedCustomers.length === 0 ? (
                <div className="text-center py-12 text-gray-500">
                  <p>No approved customers found{filterType !== 'All' ? ` for ${filterType}` : ''}.</p>
                </div>
              ) : (
                <>
                  <div className="mb-4 text-sm text-gray-600">
                    Showing {approvedCustomers.length} of {pagination.total} approved customers
                    ...
                    <table className="min-w-full border-collapse">
                      <thead>
                        <tr className="bg-gray-50">
                          ...
                          <th className="border border-gray-300 px-3 py-2 text-left text-xs font-semibold text-gray-700">Pending</th>
                          ...
                          <tr key={customer.id} className="hover:bg-gray-50">
                            ...
                            <td className="border border-gray-300 px-3 py-2 text-xs">
                              <button
                                ...
                                disabled={loading}
                                className="bg-yellow-600 text-white px-3 py-1 rounded text-xs hover:bg-yellow-700 transition-colors w-full disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                Pending
                              </button>
                            </td>
